<?php
    ...
    class Persons implements Table {
        ...
        // CONSTRUCTOR
        function __construct($id, $name, $email, $mobile, $pic) {
            $this->id     = $id;
            $this->name   = $name;
            $this->email  = $email;
            $this->mobile = $mobile;
        ...
        public function displayListScreen() {
            ...
            foreach (Database::prepare('SELECT * FROM `tt_persons`', array()) as $row) {
                echo "
                    <tr>
                        ...
                        <td>{$row['mobile']}</td>
                        <td>
						<img src=\"data:image/jpeg;base64," . base64_encode( $row['pic'] ) . "\"/></td>
                        <td>
            ...
            echo "</tbody></table>
                            <div class='form-actions'>
                                <a class='btn' href='../home.html'>Back</a><a class='btn btn-danger' href='../logout.php'>Logout</a>
                            </div>
				</div></div></div>";
        }
        ...
        public function createRecord() {
			echo $this->pic;
			echo var_dump($this->pic);
			
			
			  $fileName = $_FILES[$this->pic]['name'];
  $tmpName  = $_FILES[$this->pic]['tmp_name'];
  $fileSize = $_FILES[$this->pic]['size'];
  $fileType = $_FILES[$this->pic]['type'];
  $fileType = (get_magic_quotes_gpc()==0 
    ? mysql_real_escape_string($_FILES[$this->pic]['type'])
    : mysql_real_escape_string(stripslashes ($_FILES[$this->pic])));
  $fp       = fopen($tmpName, 'r');
  ...
  $content  = addslashes($content);
            ...
            if ($this->validate()) {	
                Database::prepare(
					"INSERT INTO tt_persons (name, email, mobile, pic) VALUES (?,?,?,?)",
                    array($this->name, $this->email, $this->mobile, $content)
                );
                    ...
                    <div class='span10 offset1'>
                        ...
                        <div class='form-horizontal'>
                            ...
                            <div class='control-group'>
                                ...
                                <div class='controls'>
                                    <label class='checkbox'>
										<img src=\"data:image/jpeg;base64," . base64_encode( $rec['pic'] ) . "\"/>
                                    </label>
                                </div>
                            </div>
                            <div class='form-actions'>
        ...
        public function updateRecord() {
            if ($this->validate()) {
                Database::prepare(
                    "UPDATE tt_persons SET name = ?, email = ?, mobile = ?, pic = ? WHERE id = ?",
                    array($this->name, $this->email, $this->mobile, $this-> pic, $this->id)
                );
                $this->displayListScreen();
            } else {
